...
class RunControllerTest {
    ...
    @BeforeEach
    void setUp(){
        runs.add(new Run(1,"monday run", LocalDateTime.now(),LocalDateTime.now().plusHours(1),3,Location.INDOOR));
    }

    @Test
    void shouldFinAllRuns() throws Exception {
        when(repository.findAll()).thenReturn(runs);
        mock.perform(get("/api/runs"))
    ...
    void shouldReturnNotFoundWithInvalidId() throws Exception {
        mock.perform(get("/api/runs/99"))
                .andExpect(status().isNotFound());
    }

    @Test
    void shouldCreateNewRun() throws Exception {
        var run = new Run(null,"test", LocalDateTime.now(),LocalDateTime.now().plusHours(1),1, Location.INDOOR);
        mock.perform(post("/api/runs")
    ...
    void shouldUpdateRun() throws Exception {
        var run = new Run(null,"test", LocalDateTime.now(),LocalDateTime.now().plusHours(2),1, Location.INDOOR);
        mock.perform(put("/api/runs/1")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(run))
                )
    ...
    public void shouldDeleteRun() throws Exception {
        mock.perform(delete("/api/runs/1"))
                .andExpect(status().isNoContent());
    }
